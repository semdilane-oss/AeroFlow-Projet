...
if st.session_state["user_role"] == "passager":
    ...
    # Micro positionné proprement au-dessus de l'input de chat natif
    langue_stt = "fr-FR" if langue_interface == "Français" else "en-US"
    c_mic_pax1, c_mic_pax2, c_mic_pax3 = st.columns([3, 2, 3])
    with c_mic_pax2:
        texte_vocal_pax = speech_to_text(
            start_prompt="🎙️ Cliquer pour parler",
...
# ------------------------------------------------------------------------------
    ...
    with st.sidebar:
        st.markdown("---")
        st.write(f"🛡️ {t('Agent connecté :', 'Connected Agent:')} **{st.session_state['current_user']}**")
        if st.button(t("Déconnexion Sécurisée", "Secure Sign Out")):
            st.session_state["user_role"] = None
            st.rerun()

        st.markdown("---")
        st.subheader(t("📂 Données de vol (AIGE)", "📂 Flight Data (AIGE)"))
        fichier_importe = st.file_uploader(
            t("Charger le programme des vols (CSV)", "Upload flight schedule (CSV)"),
            type=["csv"]
        )
        ...
        if "df_vols" not in st.session_state:
            ...
            if fichiers_csv_locaux:
                ...
                st.error(t("⚠️ Aucun fichier CSV trouvé dans le répertoire.", "⚠️ No CSV file found in the repository."))
                st.stop()
        ...
        st.metric(
            label=t("🤖 Capacité Estimée (Dynamique)", "🤖 Estimated Capacity (Dynamic)"),
            value=f"{capacite_agent_heure} {t('pax/h/agent', 'pax/h/agent')}"
        )
    ...
    with c4:
        ...
        color_val = "#EF4444" if len(vols_critiques) > 0 else title_color
    ...
    with col_left:
        ...
        if "Tranche_Horaire" in df.columns and "Passagers" in df.columns:
            ...
            fig_affluence.update_layout(
                xaxis_title=t("Tranche Horaire", "Time Slot"),
                yaxis_title=t("Total Passagers", "Total Passengers"),
    ...
    with col_right:
        ...
        if "Temps_Escale_Min" in df.columns:
            ...
            fig_transit.update_layout(
                xaxis_title=t("Plage de Temps d'Escale", "Layover Time Range"),
                yaxis_title=t("Nombre de Vols", "Number of Flights"),
                margin=dict(l=10, r=10, t=30, b=10),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)"
            )
    ...
    st.markdown("---")
    st.subheader(t("⚠️ Centre d'Alertes et Annonces Vocales", "⚠️ Alerts & Voice Announcements Center"))
    ...
    if len(vols_critiques) > 0:
        ...
        with col_btn:
            langue_choisie = st.radio(
                t("🌐 Langue de l'annonce vocale :", "🌐 Voice announcement language:"),
                options=["Français", "English"],
                horizontal=True,
                key="choix_langue_audio"
            )
            nb_crit = len(vols_critiques)
            total_pax_crit = int(vols_critiques["Passagers_Transit"].sum()) if "Passagers_Transit" in vols_critiques.columns else 0
            ...
            try:
                ...
                st.info(f"{t('Annonce audio générée avec succès.', 'Audio announcement successfully generated.')}")
            except Exception as e:
        ...
        st.success(t("✅ Aucun risque de correspondance détecté pour le moment sur le réseau AIGE.", "✅ No connection risk detected at the moment on the AIGE network."))
    ...
    with exp_col1:
        ...
        if not vols_critiques.empty:
            st.download_button(
                label=t("📄 Télécharger Vols Critiques (.csv)", "📄 Download Critical Flights (.csv)"),
                data=vols_critiques.to_csv(index=False, encoding="utf-8-sig"),
    ...
    with exp_col2:
        ...
        st.download_button(
            label=t("📊 Télécharger Programme Complet (.csv)", "📊 Download Full Schedule (.csv)"),
    ...
    with exp_col3:
        ...
        if REPORTLAB_AVAILABLE:
            pdf_bytes = generer_pdf_rapport(df, vols_critiques, total_passagers, total_transit, guichets_ouverts)
            st.download_button(
                label=t("📑 Télécharger le Rapport (.pdf)", "📑 Download Report (.pdf)"),
                data=pdf_bytes,
    ...
    # ------------------------------------------------------------------------------
    # 7. SECTION CHATBOT INTELLIGENT BILINGUE INTÉGRÉ (ESPACE AGENT)
    # ------------------------------------------------------------------------------
    st.markdown("---")
    st.subheader(t("💬 Assistant Virtuel AeroFlow (Chatbot Bilingue - Agents)", "💬 AeroFlow Virtual Assistant (Bilingual Chatbot - Agents)"))
    ...
    if not st.session_state["messages_chat"]:
        st.session_state["messages_chat"] = [
            {"role": "assistant", "content": t("Bonjour ! Je suis l'assistant intelligent d'AeroFlow. Comment puis-je vous aider dans la régulation aujourd'hui ?", "Hello! I am AeroFlow's intelligent assistant. How can I help you with regulation today?")}
        ]
    ...
    c_mic_agent1, c_mic_agent2, c_mic_agent3 = st.columns([3, 2, 3])
    with c_mic_agent2:
    ...
    if prompt_utilisateur:
        ...
        if any(w in p_lower for w in ["critique", "critical", "delay", "retard"]):
            ...
            reponse_bot = f"There are currently {nb_c} critical flight(s) with a layover of 45 minutes or less." if est_anglais else f"Il y a actuellement {nb_c} vol(s) critique(s) avec un temps d'escale inférieur ou égal à 45 minutes."
        elif any(w in p_lower for w in ["passager", "pax", "passenger", "total"]):
            reponse_bot = f"Expected passenger traffic today is {total_passagers:,} passengers, including {total_transit:,} in transit." if est_anglais else f"Le trafic passagers attendu aujourd'hui est de {total_passagers:,} passagers, dont {total_transit:,} en transit."
        elif any(w in p_lower for w in ["guichet", "counter", "agent", "capacity"]):
            reponse_bot = f"There are {guichets_ouverts} counters currently open on-site, with an estimated processing capacity of {capacite_agent_heure} pax/h/agent." if est_anglais else f"Il y a {guichets_ouverts} guichets ouverts sur le terrain, avec une capacité de traitement estimée à {capacite_agent_heure} pax/h/agent."
        else:
            reponse_bot = "I am AeroFlow's assistant. You can ask me about passenger numbers, critical flights, or counter capacity at AIGE airport!" if est_anglais else "Je suis l'assistant d'AeroFlow. Vous pouvez m'interroger sur le nombre de passagers, les vols critiques ou la capacité des guichets à l'AIGE !"
